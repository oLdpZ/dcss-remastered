...
cmd /c "`"$vc`" x86 && cl /nologo /LD /O2 `"$here\gl_proxy.c`" `"$here\postprocess.c`" `"$here\iat_hook.c`" `"$here\file_hook.c`" `"$here\shmem.c`" /Fe:`"$here\opengl32.dll`" /link /MACHINE:X86 gdi32.lib user32.lib opengl32.lib psapi.lib"
